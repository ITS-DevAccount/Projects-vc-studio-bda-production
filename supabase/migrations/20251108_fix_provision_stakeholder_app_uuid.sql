-- Fix provision_stakeholder to remove app_uuid from stakeholders table operations
DROP FUNCTION IF EXISTS provision_stakeholder(jsonb,text[],uuid,jsonb,uuid,text,boolean,uuid,uuid);

CREATE OR REPLACE FUNCTION provision_stakeholder(
    p_stakeholder JSONB,
