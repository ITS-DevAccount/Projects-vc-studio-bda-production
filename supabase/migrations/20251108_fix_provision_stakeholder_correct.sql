-- Drop all versions and create with correct signature (8 params, NO app_uuid)
...
BEGIN
    ...
    IF p_relationships IS NOT NULL THEN
        FOR v_relationship IN SELECT * FROM jsonb_array_elements(p_relationships) LOOP
            ...
            IF v_to_stakeholder_id IS NOT NULL AND v_relationship_type_id IS NOT NULL THEN
                ...
                VALUES (v_stakeholder_id, v_to_stakeholder_id, v_relationship_type_id, v_strength, 'active')
                ON CONFLICT (from_stakeholder_id, to_stakeholder_id, relationship_type_id) DO UPDATE SET
                    strength = EXCLUDED.strength, status = 'active', updated_at = NOW();

                IF v_bidirectional THEN
                    INSERT INTO relationships (from_stakeholder_id, to_stakeholder_id, relationship_type_id, strength, status)
                    VALUES (v_to_stakeholder_id, v_stakeholder_id, v_relationship_type_id, v_strength, 'active')
                    ON CONFLICT (from_stakeholder_id, to_stakeholder_id, relationship_type_id) DO UPDATE SET
                        strength = EXCLUDED.strength, status = 'active', updated_at = NOW();
                END IF;
            END IF;
    ...
    UPDATE stakeholders SET core_config = v_core_config, updated_at = NOW() WHERE id = v_stakeholder_id;

    -- Create workspace node (no app_uuid)
    ...
    -- Create welcome notification (no app_uuid)
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'notifications') THEN
        ...
        VALUES (v_stakeholder_id, 'welcome', 'Welcome!', v_welcome_message, false)
        RETURNING id INTO v_notification_id;
    END IF;

    -- Create workflow instance (no app_uuid)
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'workflow_instances') THEN
        ...
        -- Create welcome activity (no app_uuid)
